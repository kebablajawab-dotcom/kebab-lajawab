...
import { Image as ImageIcon, Loader2, Plus, Trash2, X, Pin, PinOff } from 'lucide-react';
...
const Gallery: React.FC<GalleryProps> = ({ isAdmin, onViewMore }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      // If index is missing, fallback to simple sorting
      if (error.message.includes('index')) {
        ...
        onSnapshot(simpleQ, (snap) => {
          ...
          // Manual sort for pinning if index fails
          items.sort((a, b) => {
  ...
  return (
    <section id="gallery" className="py-24 bg-charcoal">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-serif font-bold mb-4 text-gold">Restaurant Gallery</h2>
          <div className="w-24 h-1 bg-gold mx-auto mb-6"></div>
          <p className="text-white/60 max-w-2xl mx-auto">
            A glimpse into the heart of Kebab Lajawab. Authentic flavors, warm ambiance, and unforgettable memories.
          </p>
        </motion.div>
        ...
        {hasMore && (
          <div className="mt-12 text-center">
            <button
              onClick={onViewMore}
              className="px-8 py-3 border-2 border-gold text-gold font-bold rounded-full hover:bg-gold hover:text-charcoal transition-all transform hover:scale-105"
            >
              View More Images
            </button>
          </div>
        ...
        {images.length === 0 && !loading && (
          <div className="text-center py-20 border-2 border-dashed border-white/10 rounded-2xl">
            ...
            <p className="text-white/40">No gallery images yet. {isAdmin ? 'Add some to get started!' : ''}</p>
          </div>
        )}
      </div>
